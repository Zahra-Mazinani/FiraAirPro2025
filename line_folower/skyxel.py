...
def gate_center(frame):
    ...
    if len(stats) > 1:  # چک کردن اینکه آیا حداقل یک جسم پیدا شده
        # filtered_stats = [(i, stat) for i, stat in enumerate(stats[1:], start=1) if stat[cv2.CC_STAT_AREA] >= min_area]
        
        # # پیدا کردن بزرگترین مساحت به جز پس‌زمینه (index 0)
        # largest_index = np.argmax(filtered_stats[1:, cv2.CC_STAT_AREA]) + 1
        
        filtered_stats = [(i, stat) for i, stat in enumerate(stats[1:], start=1) if stat[cv2.CC_STAT_AREA] >= min_area]
        if filtered_stats:
            largest_index, _ = max(filtered_stats, key=lambda item: item[1][cv2.CC_STAT_AREA])
